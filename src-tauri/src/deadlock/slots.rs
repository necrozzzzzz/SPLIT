...
pub(crate) fn preset_history_snapshot(preset: u8) -> Result<(String, Vec<SlotEntry>), String> {
    if !(1..=PRESET_COUNT as u8).contains(&preset) {
        return Err(format!("Invalid preset {preset}"));
    }
...
pub(crate) fn apply_preset_history_snapshot(
    preset: u8,
    name: String,
    ...
    if entries.len() != SLOT_COUNT {
        return Err(format!(
            "Invalid preset history slot count: expected {SLOT_COUNT}, got {}",
            entries.len(),
        ));
    }

    if name.trim().is_empty() {
        return Err("Preset history name cannot be empty".to_string());
    }
